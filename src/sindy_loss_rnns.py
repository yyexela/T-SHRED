...
class SINDyLossGRU(nn.Module):
    def __init__(self, d_model:int = 3,
                 num_layers:int = 2,
                 dropout:float = 0.1,
                 poly_order: int = 2,
        ...
        # SINDy components
        self.library_dim = calculate_library_dim(self.d_model, self.poly_order, self.include_sine)

        # SINDy coefficients (learnable parameters)
        self.coefficients = nn.Parameter(torch.Tensor(self.library_dim, self.d_model))
        ...
        self.register_buffer('coefficient_mask', torch.ones(self.library_dim, self.d_model))
