...
def plot_field_comparison(
    ...
    for i in range(n_dims):
        ...
        if n_dims >= 2:
            ax_pred.set_title(
                f"Prediction (dim {i}: {planetswe_fields[i]})", fontsize=title_fontsize
            )
        else:
        ...
        if n_dims >= 2:
            ax_target.set_title(
                f"Target (dim {i}: {planetswe_fields[i]})", fontsize=title_fontsize
            )
        else:
        ...
        if n_dims >= 2:
            ax_error.set_title(
                f"Absolute Error (dim {i}: {planetswe_fields[i]})",
...
def plot_model_results_scatter(
    ...
    if results_type == "hyper_opt":
        filtered_results = [
            r for r in results if r["final_config"]["model"]["dataset"] == dataset
        ]
    elif results_type == "pickle":
        filtered_results = [
            r for r in results if r["hyperparameters"]["dataset"] == dataset
        ]
    else:
    ...
    for r in filtered_results:
        if results_type == "hyper_opt":
            if (
                ...
                and r["final_config"]["model"]["encoder"] not in encoders
            ):
                continue
        elif results_type == "pickle":
    ...
    unique_encoders = list(
        set([r["hyperparameters"]["encoder"] for r in aggregated_results])
    )
    unique_decoders = list(
    ...
    encoder_color_map = {
        encoder: encoder_colors[i % len(encoder_colors)]
        for i, encoder in enumerate(unique_encoders)
    }

    # Create marker symbol mappings for decoders
    decoder_symbol_map = {"mlp": "circle", "cnn": "triangle-up"}
    ...
    for encoder in unique_encoders:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="markers",
                name=f"{encoder_name}",
                marker=dict(color=encoder_color_map[encoder], size=12, symbol="square"),
                showlegend=True,
            )
        )
    ...
    fig.update_layout(
        title=dict(text=f"{dataset_name}", font=dict(size=title_fontsize)),
        xaxis_title="Model",
        ...
        xaxis=dict(
            title=dict(
                ...
                font=dict(size=axes_fontsize),
            ),
            showticklabels=False,
            tickfont=dict(size=axes_fontsize),
        ),
        yaxis=dict(
            type="log",
            title=dict(text="Test Loss", font=dict(size=axes_fontsize)),
            exponentformat="E",
            tickformat="1.2e",
            nticks=5,
            tickfont=dict(size=axes_fontsize),
        ...
        legend=dict(
            title=dict(text="Legend", font=dict(size=legend_fontsize)),
...
def plot_eigvs(eigvs_np, save: bool = False, fname: str = None) -> None:
    ...
    for i in range(terms):
        ...
        ax.plot(real_traj, imag_traj, color="gray", linewidth=1, alpha=0.5, zorder=1)
    ...
    if steps > 1:
        ...
        legend_elements.append(
            Line2D([0], [0], color="gray", alpha=0.5, label="Trajectory")
        )
    else:
        legend_elements.append(
            Line2D(
