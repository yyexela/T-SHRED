...
cmd_template = \
"""\
time python -u {script_dir} --dataset {dataset} --device {device} --encoder {encoder} --decoder {decoder} --decoder_depth {decoder_depth} --dropout {dropout} --epochs {epochs} --save_every_n_epochs {save_every_n_epochs} --hidden_size {hidden_size} --lr {lr} --n_heads {n_heads} --poly_order {poly_order} --batch_size {batch_size} --encoder_depth {encoder_depth} --input_length {input_length} --early_stop {early_stop} --sindy_attention_weight {sindy_attention_weight} --n_well_tracks {n_well_tracks} --generate_test_plots --seed {seed} --identifier {identifier} --verbose 2>&1 | tee {log_path}
"""
