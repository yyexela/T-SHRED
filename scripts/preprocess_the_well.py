...
n_steps = {
    "planetswe": 1008,
    "active_matter": 81,
    "gray_scott_reaction_diffusion": 1001,
}
total_tracks = {
    "planetswe": 10,
    "active_matter": 360,
    "gray_scott_reaction_diffusion": 1200,
}
...
full_mat = create_mats_full(
    train_data, valid_data, test_data, total_tracks[dataset], debug=debug
)

print(
    "full_mat.shape:", full_mat.shape
)  # (n_steps * n_tracks, im_rows, im_cols, im_dim)
...
for track_num in tqdm(range(total_tracks[dataset]), desc="Saving full data"):
    ...
    track = full_mat_scaled[
        track_start_idx:track_end_idx, :
