    ...
    with torch.no_grad():
        for i in range(len_ds):
            ...
            if hp.dataset == "plasma":
                ...
                plots.plot_field_comparison(outputs_shaped, labels, dataset=hp.dataset, save=True, fname=f"{hp.encoder}_{hp.decoder}_{hp.dataset}_e{hp.encoder_depth}_d{hp.decoder_depth}_lr{hp.lr:0.2e}_full_comparison")

                # Only do first trajectory
                break
                ...
                # Generate plots
                plots.plot_field_comparison(pod_outputs_shaped, pod_labels_shaped, dataset=hp.dataset, save=True, fname=f"{hp.encoder}_{hp.decoder}_{hp.dataset}_e{hp.encoder_depth}_d{hp.decoder_depth}_lr{hp.lr:0.2e}_pod_comparison")
                plots.plot_field_comparison(pod_outputs_shaped, full_labels_shaped, dataset=hp.dataset, save=True, fname=f"{hp.encoder}_{hp.decoder}_{hp.dataset}_e{hp.encoder_depth}_d{hp.decoder_depth}_lr{hp.lr:0.2e}_full_comparison")
